...
统一 Task 系统用于追踪所有异步执行单元。合并了原有的 Agent-Loop Task 和 Multi-Agent Task，形成统一的任务管理基础设施。
...
| `local_bash` | 被设置为后台执行的 bash tool 对应的 Task |
...
AgentTool 创建异步子 Agent（或同步 Agent 超时转异步）时，自动创建 `local_agent` 类型的 Task。子 Agent 完成后，通过回调将 Task 状态更新为 `completed`。
...
├── models.py           # TaskType, TaskStatus, Task, AgentCompletionEvent
