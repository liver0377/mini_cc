...
_MAX_PRIOR_WORK_ITEM_SUMMARY_CHARS = 1200
_AGENT_HEARTBEAT_INTERVAL_SECONDS = 30.0
...
class _DiagRef:
    ...
    def __init__(self) -> None:
        self.value: QueryDiagnostics | None = None


_current_diag_ref: contextvars.ContextVar[_DiagRef] = contextvars.ContextVar("step_runner_diag_ref")
...
class StepRunner:
    ...
    async def run_step(self, step: Step, run_state: RunState) -> StepResult:
        ...
        _current_diag_ref.set(_DiagRef())
    ...
    def _build_timeout_result(self, timeout_seconds: int) -> StepResult:
        diag = _current_diag_ref.get().value
    ...
    async def _run_query_prompt(self, prompt: str, mode: str, step: Step, run_state: RunState) -> StepResult:
        ...
        _current_diag_ref.get().value = diag
        ...
        if isinstance(layers, dict):
            for key, value in layers.items():
                if isinstance(value, bool):
                    metadata[f"audit_layer_{key}"] = str(value).lower()
                else:
                    metadata[f"audit_layer_{key}"] = str(value)
    ...
    async def _execute_agent_loop(
        ...
        *,
        ...
        pending_event: asyncio.Task[Any] | None = asyncio.create_task(iterator.__anext__())
        try:
            while pending_event is not None:
                done, _ = await asyncio.wait({pending_event}, timeout=_AGENT_HEARTBEAT_INTERVAL_SECONDS)
                if not done:
                    heartbeat = AgentHeartbeatEvent(
                        agent_id=agent_id,
                        task_id=task_id,
                        elapsed_seconds=max(0, int(time.monotonic() - diag.started_at)),
                    )
                    diag.record_event(heartbeat)
                    await self._emit_query_event(heartbeat, step, run_state)
                    continue
                try:
        ...
        for dep_id in work_item.depends_on:
            for item in step.work_items:
                if item.id == dep_id and item.summary:
                    ...
                    if len(summary) > _MAX_PRIOR_WORK_ITEM_SUMMARY_CHARS:
                        summary = summary[:_MAX_PRIOR_WORK_ITEM_SUMMARY_CHARS].rstrip() + "\n...[truncated]"
                    dep_parts.append(f"### 前置任务「{item.title}」的结果\n\n{summary}")
                    break
        if not dep_parts:
            return ""
        return (
            "以下是前序任务的精简结果。请直接基于这些结果执行当前任务，不要重复已有分析：\n\n"
            + "\n\n".join(dep_parts)
            + "\n\n---\n\n"
        )
