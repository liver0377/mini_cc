...
class TestModePropagation:
    ...
    async def test_create_agent_includes_run_context_when_run_id_provided(self, tmp_path):
        project_root = tmp_path / "project"
        runs_dir = project_root / ".mini_cc" / "runs" / "run-ctx"
        runs_dir.mkdir(parents=True, exist_ok=True)
        (runs_dir / "state.json").write_text(
            '{"run_id":"run-ctx","goal":"Fix tests","status":"running","phase":"edit_code"}',
            encoding="utf-8",
        )

        task_service = TaskService(task_list_id="test-mode-runctx", base_dir=tmp_path / "tasks")
        completion_queue: asyncio.Queue[AgentCompletionEvent] = asyncio.Queue()
        prompt_builder = SystemPromptBuilder()
        env_info = EnvInfo(
        ...
        agent = await manager.create_agent(prompt="test run context", mode="plan", run_id="run-ctx")

        content = agent.state.messages[0].content or ""
        assert "Run ID: run-ctx" in content
    ...
    async def test_agent_tool_passes_run_id(self):
        manager = AsyncMock()
        state_fn = MagicMock(return_value=QueryState())
        mode_fn = MagicMock(return_value="plan")
        run_id_fn = MagicMock(return_value="run-ctx")
        tool = AgentTool(
            manager=manager,
            get_parent_state=state_fn,
            get_mode=mode_fn,
            get_run_id=run_id_fn,
        )

        agent = _make_tool_agent_mock()
        manager.create_agent = AsyncMock(return_value=agent)

        result = await tool.async_execute(prompt="test run id pass")
        assert result.success is True

        call_kwargs = manager.create_agent.call_args
        assert call_kwargs.kwargs.get("run_id") == "run-ctx"
