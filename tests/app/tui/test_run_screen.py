...
from mini_cc.app.tui.screens.run_screen import (
    _RUN_STATUS_COLORS,
    _RUN_STATUS_ICONS,
    _STEP_STATUS_ICONS,
    RunScreen,
...
class TestRunScreenDataOps:
    ...
    def test_scheduler_lines_render_scheduler_decisions(self, tmp_path):
        store = CheckpointStore(base_dir=tmp_path)
        run = _make_run_state()
        store.save_state(run)
        store.append_scheduler_decision(
            SchedulerDecisionRecord(
                ...
                selected_priority=44,
                considered_count=2,
                reason="selected step-2 as highest-priority implementer candidate",
        ...
        lines = screen._scheduler_lines(run.run_id)

        assert any("role=implementer" in line for line in lines)
        assert any("considered=2" in line for line in lines)
        assert any("rejected=step-ro" in line for line in lines)
